...
Этим шаблоном мы делимся в рамках данного репозитория. Наш вклад opensource.

## Какая проблема решается

Основной риск в начале любого проекта - не угадать со стеком и архитектурой, завязнуть в разработке рутиных задач.
...
Мы вам заменим дорогого архитектора проекта и заложим грамотную архитектуру, усточивую к плохим решениям в конкретных
модулях.

Вы себя затрахуете от очень дорогой ошибки неравильного выбора стека и архитектуры, влекующую переписку проекта с нуля,
...
4. должно быть много документации и готовых модулей под популярные задачи для сокрашения время разработки.
...
Проекты собираются фактически готовых кубиков, где минимальная еденица - это виджет, отвечающий кусочек логики на
...
2. Логин/Регистрация
3. Биллинг - оплата картой, высталение счета, зачисление на баланс закрывающие.
4. Мониторинг
5. Юнит-тесты
6. Кеширование - стандартное и статическое
7. Работа с изображениями- ресайз
...
Go изначально придумали для высокогруженных микросервисов(микропроектов), например быстро обрабатывать миллионы запросов
...
Для веб-разработки используется фреймворк django и это доволько редкий инструмент, могут быть проблемы с кадрами.

Если у вас есть команда на django - делайте на django, если команды нет - php/yii2.

### Почему не Ruby on Rails?

Устаревшая забытая технология покрайней мере в СНГ. Точно нет.
...
Можем проконсултировать как делать ваш проект или добавить недостающие модули общего назначения.
